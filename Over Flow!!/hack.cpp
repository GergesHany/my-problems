...
ll solve1(unsigned long long x,unsigned long long y){
    int d1 = 1+(int)log10(x);
    int d2= 1+(int)log10(y);
    if (d1+d2 <=20)
    {
...
void Generate_tests() {
    int t = gen_int(1e6, 1e6);
    while(t--){
        ll n = gen_int(pow(10,15), pow(10, 16)), k = gen_int(pow(10,1), pow(10,4));
